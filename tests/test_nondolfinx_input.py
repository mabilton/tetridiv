...
tri2quad_3d_test_names, tri2quad_3d_test_dirs = utils.get_mesh_file_names_and_dirs('./tests/test_meshes/nondolfinx/tri2quad/3d')
tri2quad_2d_test_names, tri2quad_2d_test_dirs = utils.get_mesh_file_names_and_dirs('./tests/test_meshes/nondolfinx/tri2quad/2d')
...
@pytest.mark.parametrize("input_mesh_type", input_types_to_test, 
                         ids=[f'inputmeshtype_{type_}' for type_ in input_types_to_test])
@pytest.mark.parametrize("req_output_mesh_type", output_types_to_test, 
                         ids=[f'outputmeshtype_{type_}' for type_ in output_types_to_test])
def test_tet2hex_no_errors(mesh_dir, input_mesh_type, req_output_mesh_type):
...
@pytest.mark.parametrize("req_output_mesh_type", output_types_to_test, 
                         ids=[f'outputmeshtype_{type_}' for type_ in output_types_to_test])
def test_tri2quad_3d_no_errors(mesh_dir, input_mesh_type, req_output_mesh_type):
    input_mesh, correct_output_mesh = utils.load_meshes(mesh_dir)
    tetridiv_inputs = utils.create_tetridiv_inputs(input_mesh, input_mesh_type, req_output_mesh_type)
...
@pytest.mark.parametrize("req_output_mesh_type", output_types_to_test, 
                         ids=[f'outputmeshtype_{type_}' for type_ in output_types_to_test])
@pytest.mark.parametrize("zero_col_idx", zero_col_idx, ids=[f'zerocolidx_{axis}' for axis in zero_col_idx])
def test_tri2quad_2d_no_errors(mesh_dir, input_mesh_type, req_output_mesh_type, zero_col_idx):
    input_mesh, correct_output_mesh = utils.load_meshes(mesh_dir)
    if zero_col_idx is not None:
        # Add additional zero col to points so it's a 3d matrix:
        input_mesh['points'] = utils.append_zero_col(input_mesh['points'], col_idx=zero_col_idx)
        correct_output_mesh['points'] = utils.append_zero_col(correct_output_mesh['points'], col_idx=zero_col_idx)
    tetridiv_inputs = utils.create_tetridiv_inputs(input_mesh, input_mesh_type, req_output_mesh_type)
    output_mesh = tetridiv.tri2quad(**tetridiv_inputs)
    assert_meshes_are_same(output_mesh, correct_output_mesh, input_mesh_type, req_output_mesh_type)
...
def assert_meshes_are_same(output_mesh, correct_output_mesh, input_mesh_type, req_output_mesh_type):
    ...
    correct_points, correct_cells = process_correct_mesh(correct_output_mesh, output_cell_geom, input_mesh_type, output_type)
