...
# List of mesh files:
tet2hex_test_names, tet2hex_test_dirs = utils.get_mesh_file_names_and_dirs('./tests/test_meshes/nondolfinx/tet2hex')
tri2quad_test_3d_names, tri2quad_test_3d_dirs = utils.get_mesh_file_names_and_dirs('./tests/test_meshes/nondolfinx/tri2quad/3d')
tri2quad_test_2d_names, tri2quad_test_2d_dirs = utils.get_mesh_file_names_and_dirs('./tests/test_meshes/nondolfinx/tri2quad/2d')
...
@pytest.mark.parametrize("mesh_dir", [*tri2quad_test_3d_dirs, *tri2quad_test_3d_dirs], 
                         ids=[*tri2quad_test_3d_names, *tri2quad_test_3d_names])
@pytest.mark.parametrize("input_mesh_type", input_types_to_test, ids=[f'input_={type_}' for type_ in input_types_to_test])
@pytest.mark.parametrize("req_output_mesh_type", output_types_to_test, ids=[f'output_={type_}' for type_ in output_types_to_test])
...
invalid_meshes = {'int': 1, 'array': np.ones((2,2)), 'str': 'abc', 'tuple': (1,'a'), 'list': [2,'b']}
@pytest.mark.parametrize("mesh", invalid_meshes.values(), 
...
def test_tet2hex_smallest_vertex_label_less_than_zero(mesh_dir):
    input_mesh, _ = utils.load_meshes(mesh_dir)
    points, cells = utils.unpack_mesh(input_mesh, input_mesh['cell_geom'], mesh_type='dict')
...
def test_too_many_inputs(mesh_dir, tetridiv_method):
    input_mesh, _ = utils.load_meshes(mesh_dir)
    with pytest.raises(ValueError, match='specify either mesh OR points and cells, but not both'):
        tetridiv_method(mesh=input_mesh, points=input_mesh['points'], cells=input_mesh['cells'])

@pytest.mark.parametrize("tetridiv_method", tetridiv_methods.values(), 
                         ids=[f'method_={method}' for method in tetridiv_methods.keys()])
def test_no_inputs(tetridiv_method):
    with pytest.raises(ValueError, match='specify either mesh OR points and cells as input'):
        tetridiv_method()

invalid_outputs = ('tetgen', 'abc', 1, np.array([2]))
@pytest.mark.parametrize("invalid_output_type", invalid_outputs, 
