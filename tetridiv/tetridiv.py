...
def tet2hex(mesh: Optional[Mesh] = None, points: Optional[np.ndarray] = None, cells: Optional[np.ndarray] = None, output_type: Optional[str] = None) -> Mesh:
    r"""Subdivides tetrahedral meshes into hexahedral meshes.
...
def tri2quad(mesh: Optional[Mesh] = None, points: Optional[np.ndarray] = None, cells: Optional[np.ndarray] = None, output_type:  Optional[str] = None) -> Mesh:
    r"""Subdivides triangular meshes into quarilateral meshes.
    ...
    {'cells': array([[0, 3, 6, 4], 
     ...
     'points': array([[0.        , 0.        ],
                      ...
                      [0.33333333, 0.33333333]])}
    """
    quadrilateral_mesh = _subdivide_mesh(mesh, points, cells, output_type, elem_type='tri')
    return quadrilateral_mesh

def _subdivide_mesh(input_mesh: Optional[Mesh], points: Optional[np.ndarray], cells: Optional[np.ndarray], output_type: Optional[str], elem_type: str) -> Mesh:
    ...
    subdivided_mesh = _create_mesh(subdiv_points, subdiv_cells, input_mesh, output_type, elem_type)
...
def _identify_mesh_type(mesh: Mesh) -> str:
    r"""Returns input mesh type as string."""
    ...
    if not any(input_type_matches):
        raise TypeError(f'{class_name} meshes are not natively supported; instead, either explicitly specify ' 
...
def _deduce_output_type(output_type: Optional[str], input_type: str) -> str:
    r"""Returns output mesh type as string."""
    if output_type is None:
        ...
        if not any(output_type_matches):
            raise TypeError(f"'{output_type}' is not a valid output type; either leave output_type argument unspecified "
...
def _get_points_and_cells(mesh: Mesh, mesh_type: str, elem_type: str) -> Tuple[np.ndarray, np.ndarray]:
    r"""Gets points and cells arrays of `mesh`."""
...
def _get_cells(mesh: Mesh, mesh_type: str, elem_type: str) -> np.ndarray:
    r"""Gets cell array of `mesh`."""

    if mesh_type=='dict':
        cells = _get_cells_from_dict_mesh(mesh)
    elif mesh_type=='meshio':
        cell_key = 'tetra' if elem_type=='tet' else 'triangle'
        cells = _get_cells_from_cells_dict(mesh.cells_dict, cell_key, mesh_type, elem_type)
    elif mesh_type=='pyvista':
        import vtk
...
def _get_cells_from_dict_mesh(mesh_dict: Dict[str, np.ndarray]) -> np.ndarray:
    r"""Gets cell array from a dict-type `mesh`."""
...
def _get_cells_from_cells_dict(cells_dict: Dict[str, np.ndarray], key: str, mesh_type: str, elem_type: str) -> np.ndarray:
    r"""Gets cell array from cell dictionary of `mesh`."""
...
def _get_cells_from_dolfinx_mesh(mesh: Mesh, elem_type: str) -> np.ndarray:
    r"""Gets cell array from dolfinx-typed `mesh`."""
    ...
    if elem_type not in actual_elem_type:
        elem_name = 'tetrahedral' if elem_type == 'tet' else 'triangular'  
        raise ValueError(f'dolfinx mesh consists of {actual_elem_type} cells, not {elem_name} cells. ' 
                         f'Are you sure you provided a {elem_name} mesh?')
...
def _ensure_cell_idxs_are_ints(cells: np.ndarray, epsilon: float = 1e-6) -> np.ndarray:
    r"""Converts `cells` array to `int`-typed array."""
...
def _check_points_and_cells(coords: np.ndarray, cells: np.ndarray, elem_type: str) -> None:
...
def _subdivide_points_and_cells(points: np.ndarray, cells: np.ndarray, elem_type: str) -> Tuple[np.ndarray, np.ndarray]:
    r"""Subdivides points and cells arrays of tetrahedral meshes and triangular meshes into hexahedral and quadrilateral meshes respectively.
    ...
    features_global_idxs = {feature_name: cells[:,vert_combo] for feature_name, vert_combo in local_vert_combos.items()}
    ...
    for feature, feat_pts in features_global_idxs.items():
        ...
        reshaped_feat_pts = feat_pts.reshape(-1, feat_pts.shape[-1]) # shape = (num_features_per_elem*M, v)
        # Sort vertex numbers within each element:
        sorted_feat_pts = np.sort(reshaped_feat_pts, axis=-1)
        # Remove repeated features:
        unique_feat_pts, unique_inv = np.unique(sorted_feat_pts, axis=0, return_inverse=True) # shape = (num_unique_features, v)
        
        # Step 2:
        unique_feat_pts_coords = points[unique_feat_pts,:] # shape = (num_unique_features, v, d)
        # Center of feature = average position of vertices in feature
        new_verts_coords.append(np.mean(unique_feat_pts_coords, axis=1)) # shape = (num_unique_features, d)
        ...
        repeated_new_verts = new_vert_idx[unique_inv]  # shape = (M*num_features_per_elem,)
        ...
        new_verts[feature] = repeated_new_verts.reshape(*feat_pts.shape[:-1]) # shape = (M, num_features_per_elem)
    
    # Step 5:
    # New mesh coords = old mesh coords + coords of new vertices:
    subdivided_points = np.concatenate([points, *new_verts_coords], axis=0) # shape = (N+sum(num_new_vert), d)
    ...
    if elem_type == 'tet':
        ...
        subdivided_cells = _assemble_hexahedrons(orig_verts, new_verts, local_vert_combos) # shape = (4M, 6)
    else:
        # Global idxs of original verts in each tri = verts of tri faces:
        orig_verts = features_global_idxs['face'][:,0,:] # shape = (M, 3)
        subdivided_cells = _assemble_quadrilaterals(orig_verts, new_verts, local_vert_combos) # shape = (3M, 4)
    
    return subdivided_points, subdivided_cells

def _create_vert_combos(elem_type: str) -> Dict[str, List[Tuple[int, ...]]]:
    f"""Creates dictionary of local vertex indice combinations for each feature (i.e. edge, face, volume) in a mesh element."""
...
def _order_verts_in_tet_by_height(points: np.ndarray, cells: np.ndarray, local_vert_combos: Dict[str, List[Tuple[int, ...]]]) -> np.ndarray:
    f"""Orders each row in cells array so that vertex indices appear in order of increasing z position."""
...
def _assemble_hexahedrons(orig_verts: np.ndarray, new_verts: Dict[str, np.ndarray], local_vert_combos: Dict[str, List[Tuple[int, ...]]]) -> np.ndarray:
    r"""Creates cells array of subdivided hexahedron mesh.
    ...
    num_vert_in_vol = 4
    verts = {f'{vert_idx}': orig_verts[:,vert_idx] for vert_idx in range(num_vert_in_vol)}
    ...
    subdivided_cells[0::hpt,:] = np.stack([verts['0'], edges['01'], faces['012'], edges['02'],
    ...
    subdivided_cells[1::hpt,:] = np.stack([verts['1'], edges['12'], faces['012'], edges['01'],
    ...
    subdivided_cells[2::hpt,:] = np.stack([verts['2'], edges['02'], faces['012'], edges['12'],
    ...
    subdivided_cells[3::hpt,:] = np.stack([edges['03'], faces['013'], vols['0123'], faces['023'],
                                           verts['3'], edges['13'], faces['123'], edges['23']], axis=-1)
    
    return subdivided_cells
    
def _assemble_quadrilaterals(orig_verts: np.ndarray, new_verts: Dict[str, np.ndarray], local_vert_combos: Dict[str, List[Tuple[int, ...]]]) -> np.ndarray:
    r"""Creates cells array of subdivided quadrilateral mesh.
    ...
    # Get global idx of original verts first:
    num_vert_in_face = 3 
    verts = {f'{vert_idx}': orig_verts[:,vert_idx] for vert_idx in range(num_vert_in_face)}
    # verts dictionary is of the form:
    ...
    subdivided_cells[0::qpt,:] = np.stack([verts['0'], edges['01'], faces['012'], edges['02']], axis=-1)
    # Quadrilateral 1 = Vert_1, Edge_12, Face_012, Edge_01
    subdivided_cells[1::qpt,:] = np.stack([verts['1'], edges['12'], faces['012'], edges['01']], axis=-1)
    # Quadrilateral 2 = Face_012, Edge_12, Vert_2, Edge_02
    subdivided_cells[2::qpt,:] = np.stack([faces['012'], edges['12'], verts['2'], edges['02']], axis=-1)
...
def _create_mesh(points: np.ndarray, cells: np.ndarray, mesh: Mesh, output_type: str, elem_type: str) -> Mesh:
    f"""Creates mesh of type `output_type` from provided `points` and `cells` arrays."""
    ...
    if output_type == 'dict':
        ...
        try:
            ...
            raise ImportError('pyvista and vtk must be installed to output a pyvista mesh.')
        ...
        try:
            ...
            raise ImportError('dolfinx and mpi4py must be installed to output a dolfinx mesh.')
